...
inline float BYTETracker::l2(std::vector<float> &tlwh1, std::vector<float> &tlwh2)
{
    float dx, dy, dw, dh;
...
std::vector< std::vector<float> > BYTETracker::l2_distance( std::vector<STrack*> &atracks,  std::vector<STrack> &btracks, int &dist_size, int &dist_size_size)
{
    std::vector< std::vector<float> > cost_matrix;
    if (atracks.size() * btracks.size() == 0)
    {
    ...
    for (int i = 0; i < atracks.size(); i++)
    {
        ...
        {
            float _dis;
            _dis = l2(atracks[i]->tlwh, btracks[j].tlwh);
            _iou.push_back(_dis);
        }
        cost_matrix.push_back(_iou);
    }
