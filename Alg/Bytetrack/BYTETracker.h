...
public:
	...
	void update(  std::vector<TFLPatchResult>& objects, cv::Mat &frame, std::vector<TFLtrack>& tflTracks);
...
private:
	...
	void linear_assignment( std::vector< std::vector<float> > &cost_matrix, int cost_matrix_size, int cost_matrix_size_size, float thresh,
	 ...
	 std::vector< std::vector<float> > l2_distance( std::vector<STrack*> &atracks,  std::vector<STrack> &btracks, int &dist_size, int &dist_size_size);
	 inline float l2( std::vector<float> &atlbrs,  std::vector<float> &btlbrs );
